...
<div class="row<?php echo $row_classes; ?>">
	...
	<div class="row-inner">
		...
		<?php 
		$sortable = apply_filters('nestpages_post_sortable', true, $this->post, $this->post_type);
		...
		<a href="<?php echo apply_filters('nestpages_edit_link', get_edit_post_link(), $this->post); ?>" class="page-link page-title">
			<span class="title">
				<?php 
					echo apply_filters( 'the_title', $this->post->title, $this->post->id, $view = 'nestpages_title' ); 
			...
			<?php 
				...
				if ( $user = wp_check_post_lock($this->post->id) ){
					...
					if ( $display_edit ) echo '<span class="edit-indicator">' . apply_filters('nestpages_edit_link_text', __('Edit', 'wp-nested-pages'), $this->post) . '</span>';
				}

				// Sticky
				echo '<span class="sticky"';
				$sticky_text = apply_filters('nestpages_make_sticky_text_row', __('(Sticky)', 'wp-nested-pages'), $this->post, $this->post_type);
		...
		<div class="np-post-columns">
			<ul class="np-post-info">
				<li><span class="np-author-display"><?php echo apply_filters('nestpages_post_author', $this->post->author, $this->post); ?></span></li>
		...
		<div class="action-buttons">
			...
			<div class="nestpages-dropdown" data-dropdown>
				...
				<ul class="nestpages-dropdown-content" data-dropdown-content>
					
					<?php 
					// WPML Translations
					if ( $wpml && in_array('wpml', $this->post_type_settings->row_actions) ) : ?>
					<li>
						<a href="#" data-nestpages-translations>
					...
					$include_link_dropdown = apply_filters('nestpages_include_links_dropdown', $include_link_dropdown, $this->post_type);
			...
			<?php 
			$can_quickedit_post = apply_filters('nestpages_quickedit', true, $this->post);
			...
			*/
			...
			if ( $this->post->status == 'publish' ) : 
			$link = apply_filters('nestpages_view_link', get_the_permalink(), $this->post);
			...
			<a href="<?php echo $link; ?>" class="np-btn np-view-button" target="_blank">
				<?php echo apply_filters('nestpages_view_link_text', __('View', 'wp-nested-pages'), $this->post); ?>
			</a>
			<?php 
			else :
			$link = apply_filters('nestpages_preview_link', get_the_permalink(), $this->post);
			$link = apply_filters('preview_post_link', $link, $this->post);
			?>
			<a href="<?php echo $link; ?>" class="np-btn np-view-button" target="_blank">
				<?php echo apply_filters('nestpages_preview_link_text', __('Preview', 'wp-nested-pages'), $this->post); ?>
			</a>
			<?php
			endif; // status
	...
	if ( $thumbnail_source ) :
		...
		if ( has_post_thumbnail($this->post->id) ) :
			...
			$out .= apply_filters('nestpages_thumbnail', $image, $this->post);
		else :
			$image_fallback = apply_filters('nestpages_thumbnail_fallback', false, $this->post);
			if ( $image_fallback ) :
				$out .= apply_filters('nestpages_thumbnail_fallback', $image_fallback, $this->post);
			endif;
		endif;
		$out .= '</div>';
		echo $out;
	...
	<div class="np-bulk-checkbox">
		<input type="checkbox" name="nestpages_bulk[]" value="<?php echo esc_attr($this->post->id); ?>" data-np-bulk-checkbox="<?php echo esc_attr($this->post->title); ?>" data-np-post-type="<?php echo esc_attr($this->post->post_type); ?>" />
	</div>
</div><!-- .row -->