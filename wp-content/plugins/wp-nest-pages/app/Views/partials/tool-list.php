...
<div class="nestpages-tools">
	...
	<ul class="subsubsub">
		...
		<li class="np-trash-links">
			...
			<span class="count"><a href="#" class="np-empty-trash" data-posttype="<?php echo esc_attr($this->post_type->name); ?>" data-nestpages-modal-toggle="np-trash-modal"><?php echo sprintf(__('Empty (%s)', 'wp-nested-pages'), absint($trashedCount)); ?></a></span>
		</li>
		<?php endif; ?>
		...
		<li>
			 |
			<a href="<?php echo nestpages\Helpers::defaultPagesLink($this->post_type->name); ?>">
				<?php echo apply_filters('nestpages_default_submenu_text', sprintf(__('Default %s', 'wp-nested-pages'),$this->post_type->labels->name), $this->post_type); ?>
			</a>
		</li>
		<?php endif; ?>
	</ul>
	...
	<div class="np-tools-primary">
		<form action="<?php echo admin_url('admin-post.php'); ?>" method="post" class="np-tools-sort">
			...
			<div class="select">
				<select id="np_sortauthor" name="np_author" class="nestpages-sort">
					<?php
			...
			?>
			<div class="select">
				<select id="np_orderby" name="np_orderby" class="nestpages-sort">
					<?php
						$options = array(
			...
			?>
			<div class="select">
				<select id="np_order" name="np_order" class="nestpages-sort">
					<?php
						$options = [
			...
			<?php 
				...
				foreach ( $taxonomies as $tax ) :
					if ( $this->post_type_repo->sortOptionEnabled($this->post_type->name, $tax->name, true) ) :
						...
						$out .= '<select id="np_taxonomy_' . $tax->name . '" name="' . $tax->name . '" class="nestpages-sort">';
			...
			<div class="select">
				...
				<input type="submit" id="nestpages-sort" class="button" value="<?php echo esc_attr__('Apply', 'wp-nested-pages'); ?>">
			</div>
		</form>
	</div>
	<?php endif; ?>
	...
	<div class="np-tools-primary">	
		<form action="<?php echo admin_url('admin-post.php'); ?>" method="post" class="np-tools-sort">
			<div class="select">
				<select id="np_category" name="np_category" class="nestpages-sort">
					<?php
			...
			<div class="select">
				...
				<input type="submit" id="nestpages-sort" class="button" value="Apply">
			</div>
		</form>
	...
	<div class="np-tools-search">
		<form action="<?php echo admin_url('admin-post.php'); ?>" method="post">
			...
			<input type="hidden" name="page" value="<?php echo esc_url($this->pageURL()); ?>">
			<?php wp_nonce_field('nestpages-nonce', 'nonce'); ?>
			<input type="search" name="search_term" id="nestpages-search" placeholder="<?php echo esc_attr($this->post_type->labels->search_items); ?>" <?php if ( $this->listing_repo->isSearch() ) echo ' value="' . esc_attr(sanitize_text_field($_GET['search'])) . '"'; ?>>
