...
?>
<div class="wrap nestpages <?php if ( $dark_mode ) echo 'np-dark-mode'; ?>">
	<div class="nestpages-listing-title">
		<h1 class="wp-heading-inline">
			<?php echo apply_filters('nestpages_page_title', esc_html__($this->post_type->labels->name), $this->post_type); ?>
		</h1>
		...
		<a href="<?php echo $this->post_type_repo->addNewPostLink($this->post_type->name); ?>" class="page-title-action">
			<?php echo apply_filters('nestpages_add_new_text', esc_html__($this->post_type->labels->add_new), $this->post_type); ?>
		</a>
		<?php endif; ?>
		...
		<a href="#" class="open-bulk-modal page-title-action" title="<?php _e('Add Multiple', 'wp-nested-pages'); ?>" data-parentid="0" data-nestpages-modal-toggle="np-bulk-modal">
			<?php echo apply_filters('nestpages_add_multiple_text', esc_html__('Add Multiple', 'wp-nested-pages'), $this->post_type); ?>
		</a>
		<?php endif; ?>
		...
		<a href="#" class="open-redirect-modal page-title-action" title="<?php _e('Add Link', 'wp-nested-pages'); ?>" data-parentid="0">
			<?php echo apply_filters('nestpages_add_link_text', esc_html__('Add Link', 'wp-nested-pages'), $this->post_type); ?>
		</a>
		<?php endif; ?>

		<div class="nestpages-top-toggles">
		<?php if ( $this->post_type->hierarchical && !$this->listing_repo->isSearch() ) : ?>
		<a href="#" class="np-btn nestpages-toggleall" data-toggle="closed"><?php esc_html_e('Expand All', 'wp-nested-pages'); ?></a>
		<?php endif; ?>
		...
		<div class="np-sync-menu-cont" <?php if ( $this->confirmation->getMessage() ) echo 'style="margin-top:2px;"';?>>
			...
			<?php 
				...
				<label>
					<input type="checkbox" name="np_sync_menu" class="np-sync-menu" value="sync" <?php if ( get_option('nestpages_menusync') == 'sync' ) echo 'checked'; ?>/> 
					<?php 
	...
	</div><!-- .nestpages-top-toggles -->

	</div><!-- .nestpages-listing-title -->
	...
	<div data-nestpages-error class="updated error notice is-dismissible" style="display:none;"><p></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e('Dismiss this notice.', 'wp-nested-pages'); ?></span></button></div>

	<?php include(nestpages\Helpers::view('partials/tool-list')); ?>
	...
	<div class="nestpages">
		...
		<div class="quick-edit quick-edit-form np-inline-modal" style="display:none;">
			<?php include( nestpages\Helpers::view('forms/quickedit-post') ); ?>
		</div>
		...
		<div class="quick-edit quick-edit-form-redirect np-inline-modal" style="display:none;">
			<?php include( nestpages\Helpers::view('forms/quickedit-link') ); ?>
		</div>
		<?php endif; ?>

		<?php if ( $this->user->canPublish($this->post_type->name) || $this->user->canSubmitPending($this->post_type->name) ) : ?>
		<div class="new-child new-child-form np-inline-modal" style="display:none;">
			<?php include( nestpages\Helpers::view('forms/new-child') ); ?>
		</div>
		<?php endif; ?>
	</div>
...
<?php 
include( nestpages\Helpers::view('forms/empty-trash-modal') );
include( nestpages\Helpers::view('forms/clone-form') );
if ( !$this->integrations->plugins->wpml->installed) include( nestpages\Helpers::view('forms/link-form') );
include( nestpages\Helpers::view('forms/bulk-add') );
include( nestpages\Helpers::view('forms/delete-confirmation-modal') ); 
if ( $this->integrations->plugins->wpml->installed ) include( nestpages\Helpers::view('partials/wpml-translations') );