<?php
$allowsorting = get_option('nestpages_allowsorting', []);
...
?>
<div class="nestpages-settings-general-wrapper">
	<div class="nestpages-settings-table">
		<form method="post" enctype="multipart/form-data" action="options.php">
		<?php settings_fields( 'nestpages-general' ); ?>
		<div class="row-container">
			<div class="row">
				<div class="description">
					<p><strong><?php _e('Nest Pages Version', 'wp-nested-pages'); ?></strong></p>
				</div>
				<div class="field">
					<p><?php echo get_option('nestpages_version'); ?></p>
				</div>
			</div><!-- .row -->
			...
			<div class="row">
				...
				<div class="field">
					<input type="text" name="nestpages_menu" id="nestpages_menu" value="<?php echo $this->menu->name; ?>">
				</div>
			</div><!-- .row -->
			<?php endif; ?>
			...
			<div class="row">
				...
				<div class="field">
					<p><label>
						<input type="checkbox" name="nestpages_ui[datepicker]" value="true" <?php if ( $this->settings->datepickerEnabled() ) echo 'checked'; ?> />
						<?php _e('Enable Date Picker in Quick Edit', 'wp-nested-pages'); ?>
					</label></p>
					<p><label>
						<input type="checkbox" name="nestpages_ui[non_indent]" value="true" <?php if ( $this->settings->nonIndentEnabled() ) echo 'checked'; ?> />
			...
			<div class="row">
				...
				<div class="field">
					...
					<label>
						<input type="checkbox" name="nestpages_ui[hide_menu_sync]" value="true" <?php if ( $this->settings->hideMenuSync() ) echo 'checked'; ?> />
					...
					<label>
						<input type="checkbox" name="nestpages_ui[include_private]" value="true" <?php if ( $this->settings->privateMenuEnabled() ) echo 'checked'; ?> />
					...
					<label>
						<input type="checkbox" name="nestpages_ui[manual_menu_sync]" value="true" <?php if ( $this->settings->autoMenuDisabled() ) echo 'checked'; ?> data-menu-disable-auto-checkbox />
						<?php _e('Manually sync menu.', 'wp-nested-pages'); ?>
					</label>
					</p>
					<p>
					<label>
						<input type="checkbox" name="nestpages_ui[manual_page_order_sync]" value="true" <?php if ( $this->settings->autoPageOrderDisabled() ) echo 'checked'; ?> />
						<?php _e('Manually sync page order.', 'wp-nested-pages'); ?>
					</label>
					</p>
					<p>
					<label>
						<input type="checkbox" name="nestpages_disable_menu" value="true" <?php if ( $this->settings->menusDisabled() ) echo 'checked'; ?> data-disable-menu-checkbox />
			...
			<div class="row">
				<div class="description">
					...
					<p><?php _e('Page sorting capability is also controlled through the nestpages_sorting_$type capability', 'wp-nested-pages'); ?></p>
				</div>
				<div class="field">
					<?php foreach ( $this->user_repo->allRoles() as $role ) : ?>
					<label>
						<input type="checkbox" name="nestpages_allowsorting[]" value="<?php echo $role['name']; ?>" <?php if ( in_array($role['name'], $allowsorting) ) echo 'checked'; ?> >
						<?php echo esc_html__($role['label']); ?>
					</label>
					<br />
					<?php endforeach; ?>
					<input type="hidden" name="nestpages_menusync" value="<?php echo get_option('nestpages_menusync'); ?>">
			...
			<div class="row">
				<div class="description">
					...
					<p><?php _e('Sort view access is also filterable through the nestpages_sort_view_$type filter.', 'wp-nested-pages'); ?></p>
				</div>
				<div class="field">
					<input type="hidden" name="nestpages_allowsortview[]" value="<?php echo 'administrator'; ?>" >
					...
					<label>
						...
						?>
						<input type="checkbox" name="nestpages_allowsortview[]" value="<?php echo $role['name']; ?>" <?php if ( $checked ) echo 'checked'; ?> >
						<?php echo esc_html__($role['label']); ?>
					</label>
					<br />
					<?php endforeach; ?>
			...
			<div class="row">
				<div class="description">
					...
					<p><?php _e('Warning: Resetting plugin settings will remove all menu settings, post type customizations, role customizations and any other Nest Pages settings. These will be replaced with the default settings. This action cannot be undone.', 'wp-nested-pages'); ?></p>
				</div>
				<div class="field">
					<p><button class="np-btn np-btn-trash" data-nestpages-reset-settings><?php _e('Reset Nest Pages Settings', 'wp-nested-pages'); ?></button></p>
					<div class="nestpages-reset-settings-complete" style="display:none;">
			...
			<div class="row">
				...
				<div class="field">
					<div class="nestpages-reset-user-prefs">
						<p>
							<button class="np-btn np-btn-trash" data-nestpages-reset-user-prefs><?php _e('Reset User Preferences', 'wp-nested-pages'); ?></button>
						</p>
					</div>
					<div class="nestpages-reset-user-prefs-complete" style="display:none;">
	...
	</div><!-- .nestpages-settings-table -->

	<div class="nestpages-settings-support">
		<div class="inner">
