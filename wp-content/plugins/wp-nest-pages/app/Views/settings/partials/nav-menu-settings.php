...
foreach ( $this->admin_menu_settings->roles as $role ) :
	...
	<?php include(nestpages\Helpers::view('settings/partials/nav-menu-settings/header')); ?>
		...
		<li class="np-nav-preview <?php if ( in_array($item_data['id'], $hidden) ) echo 'disabled'; ?><?php if ( $item_data['separator'] ) echo ' separator';?>" <?php if ( $item_data['separator'] ) echo 'data-np-separator-row'; ?>>
			<div class="menu-item">
				...
				<p>
					...
					<?php else : ?>
					<input type="text" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][label]" value="<?php if ( $item_data['custom_label'] ) echo $item_data['custom_label']; ?>" placeholder="<?php echo esc_html__($item_data['original_text']); ?>" class="menu-title-field" />
			...
			<div class="hide-checkbox">
				...
				<input type="checkbox" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][hidden]" value="<?php echo $item_data['id']; ?>" data-nestpages-admin-nav-item-checkbox	<?php if ( in_array($item_data['id'], $hidden) ) echo 'checked'; ?> />
				<?php endif; ?>
			</div>
			<?php else : ?>
			<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>]" value="true">
			<?php endif; ?>
			<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][order]" value="<?php echo $i; ?>" data-np-menu-order>
			<?php if ( !$item_data['separator'] ) : ?>
			<div class="np-extra-options" data-np-extra-options>
				<div class="half">
					...
					<input type="text" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][icon]" value="<?php if ( $item_data['custom_icon'] ) echo $item_data['custom_icon']; ?>" placeholder="<?php echo $item_data['original_icon']; ?>" />
				</div><!-- .half -->
				<div class="half right" style="display:none;">
					<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][link]" value="<?php echo $item_data['original_link']; ?>">
					<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][original_link]" value="<?php echo $item_data['original_link']; ?>">
				</div>
			...
			<ul class="submenu-listing" data-np-sortable-admin-subnav>
				...
				<li class="np-nav-preview <?php if ( $hidden_sub ) echo 'disabled'; ?> submenu-item" data-np-sortable-admin-nav>
					<div class="menu-item">
						...
						<div class="hide-checkbox">
							<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][submenu][<?php echo $si; ?>][label]" value="<?php echo $label; ?>" />
							<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][submenu][<?php echo $si; ?>][role]" value="<?php echo $role_name; ?>" />
							<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][submenu][<?php echo $si; ?>][link]" value="<?php echo $link; ?>" />
							<input type="hidden" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][submenu][<?php echo $si; ?>][order]" value="<?php echo $si; ?>" data-np-submenu-order>
							<?php if ( $link !== 'nested-pages-settings' ) : ?>
							<input type="checkbox" name="nestpages_admin[nav_menu_options][<?php echo $role['name']; ?>][<?php echo $item_data['id']; ?>][submenu][<?php echo $si; ?>][hidden]" value="true" data-nestpages-admin-nav-item-checkbox <?php if ( $hidden_sub )  echo 'checked'; ?>/>
							<?php endif; ?>
						</div>
					</div><!-- .menu-item -->
				</li>
