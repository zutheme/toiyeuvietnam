<?php 
namespace nestpages\Activation;
...
class Dependencies 
{
	...
	*/
	public function styles()
	{
		wp_enqueue_style(
			'nestpages', 
			$this->plugin_dir . '/assets/css/nestpages.css', 
			[], 
			$this->plugin_version
		);
	...
	* Only Loads on the Nest Pages screen
	*/
	public function scripts()
	{
		...
		if ( strpos( $screen->id, 'nestpages' ) || $settings_page ) :
			...
			if ( $np_env == 'dev' ){
				wp_enqueue_script(
					'nestpages', 
					$this->plugin_dir . '/assets/js/nestpages.js', 
					['jquery'],
					$this->plugin_version
				);
			} else {
				wp_enqueue_script(
					'nestpages', 
					$this->plugin_dir . '/assets/js/nestpages.min.js', 
					['jquery'], 
					$this->plugin_version
				);
			}
			$localized_data = [
				'np_nonce' => wp_create_nonce( 'nestpages-nonce' ),
			...
			$syncmenu = ( get_option('nestpages_menusync') == 'sync' ) ? true : false;
	...
	*/
	public function settingsScripts()
	{
		...
		if ( strpos( $screen->id, 'nested-pages-settings' ) ) :
			wp_enqueue_script(
				'nestpages-settings', 
				$this->plugin_dir . '/assets/js/nestpages.settings.min.js', 
				['jquery'], 
				$this->plugin_version
			);
		endif;
