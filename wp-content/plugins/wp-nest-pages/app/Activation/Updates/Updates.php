<?php 
namespace nestpages\Activation\Updates;

use nestpages\Entities\NavMenu\NavMenuRepository;
use nestpages\Entities\NavMenu\NavMenuSyncListing;
use nestpages\Activation\Updates\CustomFieldsToHidden;
...
class Updates 
{
	...
	*/
	...
	{
		$this->current_version = ( get_option('nestpages_version') )
			? get_option('nestpages_version') : $this->new_version;
	}

	/**
	* Add an empty Nest Pages menu if there isn't one
	* @since 1.1.5
	*/
	private function addMenu()
	{
		if ( !get_option('nestpages_menu') ){
			$menu_id = $this->nav_menu_repo->getMenuIDFromTitle('Nest Pages');
			if ( !$menu_id ) $menu_id = wp_create_nav_menu('Nest Pages');
			update_option('nestpages_menu', $menu_id);
		}
	}
	
	/**
	* Convert existing nestpages_menu option to menu ID rather than string/name
	* @since 1.1.5
	*/
	private function convertMenuToID()
	{
		if ( version_compare( $this->current_version, '1.1.5', '<' ) ){
			$menu_option = get_option('nestpages_menu');
			$menu = get_term_by('name', $menu_option, 'nav_menu');
			if ( $menu ){
				delete_option('nestpages_menu');
				update_option('nestpages_menu', $menu->term_id);
			} else {
				delete_option('nestpages_menu');
				$menu_id = wp_create_nav_menu('Nest Pages');
				update_option('nestpages_menu', $menu_id);
			}
		}
	}
	...
	*/
	...
	{	
		...
		$sql = "SELECT * FROM $options_table WHERE option_name = 'nestpages_posttypes'";
		$results = $wpdb->get_results($sql);
		if ( $results ) return;
		update_option('nestpages_posttypes', array(
			'page' => array(
				'replace_menu' => true
			)
		));
	...
	*/
	private function enabledDatepicker()
	{
		if ( version_compare( $this->current_version, '1.3.1', '<' ) ){
			$enabled = get_option('nestpages_ui', false);
			$default = [
				'datepicker' => 'true'
			];
			if ( !$enabled ) update_option('nestpages_ui', $default);
		}
	}
	...
	*/
	...
	/**
	* Convert the Nest Pages custom fields to hidden fields
	*/
