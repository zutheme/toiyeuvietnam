<?php 
namespace nestpages\Entities\PostType;
...
class PostTypeRepository 
{
	...
	*/
	...
	{
		$types = get_option('nestpages_posttypes');
	...
	*/
	public function getSinglePostType($post_type)
	{
		...
		foreach ( $row_actions as $action ){
			if ( apply_filters("nestpages_row_action_$action", true, $post_type) ) $filtered_row_actions[] = $action;
		}
	...
	*/
	public function allPostsLink($post_type)
	{
		...
		if ( $pt_object->replace_menu ){
			return esc_url( admin_url('admin.php?page=nestpages-' . $post_type) );
		}
	...
	*/
	...
	{
		return ( $post_type->name == 'page' ) ? 'nestpages' : 'nestpages-' . $post_type->name;
	}
	...
	*/
	public function getSubmenuText($post_type)
	{
		...
		return apply_filters('nestpages_sortview_text', $label, $post_type);
	}
	...
	*/
	public function quickEditStatuses($post_type)
	{
		...
		return apply_filters('nestpages_quickedit_post_statuses', $statuses, $post_type);
	}
}