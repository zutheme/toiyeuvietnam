<?php 
namespace nestpages\Entities\Post;

use nestpages\Form\Validation\Validation;
...
class PostUpdateRepository 
{
	...
	*/
	public function updateOrder($posts, $parent = 0)
	{
		...
		foreach( $posts as $key => $post )
		{
			...
			$wpdb->query( $query );
			do_action('nestpages_post_order_updated', $post_id, $parent, $key);

			if ( isset($post['children']) ) $this->updateOrder($post['children'], $post_id);
		}
		do_action('nestpages_posts_order_updated', $posts, $parent);
		return true;
	}
	...
	*/
	public function updatePost($data, $append_taxonomies = false)
	{
		...
		$this->updatenestpagesStatus($data);
	...
	*/
	...
	/**
	* Update Nest Pages Visibility (how/hide in Nest Pages interface)
	* @since 1.0
	* @param array data
	*/
	private function updatenestpagesStatus($data)
	{
	...
	/**
	* Update Nest Pages Menu Navigation Label
	* @since 1.0
	* @param array data
	*/
	...
	/**
	* Update Nest Pages Menu Navigation CSS Classes
	* @since 1.0
	* @param array data
	*/
	...
	/**
	* Update Nest Pages Menu Title Attribute
	* @since 1.0
	* @param array data
	*/
