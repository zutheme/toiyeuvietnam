...
use nestpages\Entities\Listing\Listing;
use nestpages\Entities\AdminMenu\AdminSubmenu;
use nestpages\Entities\AdminMenu\AdminSubmenuDefault;
...
class EnabledMenus 
{
	...
	*/
	private function loopEnabledTypes()
	{
		...
		foreach($this->enabled_types as $key => $type){	
			$user_can_view = apply_filters("nestpages_sort_view_$type->name", $this->user->canViewSorting($type->name), $this->user->getRoles());
	...
	*/
	private function menuPosition($c)
	{
		...
		if ( $this->post_type->name == 'post' ) return apply_filters('nestpages_menu_order', 5, $this->post_type);
		if ( $this->post_type->name == 'page') return apply_filters('nestpages_menu_order', 20, $this->post_type);
		if ( $this->post_type->menu_position ) return apply_filters('nestpages_menu_order', $this->post_type->menu_position + 1, $this->post_type);
