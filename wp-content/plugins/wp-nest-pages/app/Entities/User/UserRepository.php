...
class UserRepository
{
	...
	*/
	public function getSingleRoleCapabilities($role = 'administrator')
	{
		...
		if ( isset($wp_roles->roles[$role]) ) :
			...
			return apply_filters('nestpages_capabilities', $capabilities, $role);
		endif;
		return false;
	}
	...
	* @return boolean
	* @since 1.1.7
	* @see nestpages\Entities\User\UserCapabilities
	*/
	public function canSortPosts($post_type = 'page')
	{
		$roles = $this->getRoles();
		$user_can_sort = false;
		$roles_cansort = get_option('nestpages_allowsorting', []);
		...
		foreach($roles as $role){
			...
			if ( $role_obj->has_cap("nestpages_sorting_$post_type") ) $user_can_sort = true; // Custom Capability
		}
	...
	/**
	* Can current user view the Nest Pages Sort View
	* @return boolean
	* @since 3.1.9
	*/
