...
class SettingsRepository 
{
	...
	*/
	...
	{
		$option = get_option('nestpages_ui', false);
	...
	*/
	public function hideMenuSync()
	{
		...
		return apply_filters('nestpages_menu_sync_visible', $visible);
	}
	...
	*/
	...
	{
		$option = get_option('nestpages_menusync');
		$enabled = ( $option == 'sync' ) ? true : false;
		return apply_filters('nestpages_menu_sync_enabled', $enabled);
	}
	...
	*/
	public function autoMenuDisabled()
	{
		...
		return apply_filters('nestpages_menu_autosync_enabled', $enabled);
	}
	...
	*/
	public function privateMenuEnabled()
	{
		...
		return apply_filters('nestpages_menu_private_enabled', $enabled);
	}
	...
	*/
	...
	{
		$option = get_option('nestpages_disable_menu');
		$disabled = ( $option && $option == 'true' ) ? true : false;
		return apply_filters('nestpages_menus_disabled', $disabled);
	}
	...
	*/
	public function standardFields($post_type)
	{
		...
		$fields['hide_in_np'] = __('Hide in Nest Pages', 'wp-nested-pages');
	...
	*/
	...
	{
		$option = get_option('nestpages_admin');
	...
	*/
	public function resetSettings()
	{
		$options = [
			...
			'nestpages_allowsorting',
			'nestpages_disable_menu',
			'nestpages_menu',
			'nestpages_menusync',
			'nestpages_posttypes',
			'nestpages_ui',
	...
	*/
	...
	{
		$roles = get_option('nestpages_allowsortview');
