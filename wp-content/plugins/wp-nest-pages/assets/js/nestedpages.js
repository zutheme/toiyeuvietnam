...
nestpages.Formatter = function()
{
	...
	{
		var allButtons = $(nestpages.selectors.childToggle);
	...
	{
		$(nestpages.selectors.rows).removeClass(nestpages.cssClasses.noborder);
		$.each($(nestpages.selectors.lists), function(){
			$(this).find('.page-row:visible:first').addClass(nestpages.cssClasses.noborder);
		});
	}
	...
	{
		var amount = ( nestpages.non_indent === '1' ) ? 20 : 30;
		var indent_element = ( nestpages.non_indent === '1' ) ? '.row-inner' : '.child-toggle';
		$.each($(nestpages.selectors.lists), function(i, v){
			var parent_count = $(this).parents(nestpages.selectors.lists).length;
			var padding = 0;
			if ( !nestpages.jsData.sortable ) padding = 10;
			if ( parent_count > 0 ){
				var padding = ( parent_count * amount ) + padding;
				$(this).find(indent_element).css('padding-left', padding + 'px');
				return;
			}
			if ( !nestpages.jsData.sortable || $(this).hasClass('no-sort') ){
	...
	plugin.updatePlaceholderWidth = function(ui)
	{
		if ( nestpages.jsData.nestable ){
			...
			var listWidth = $(nestpages.selectors.sortable).width();
	...
	{
		$(nestpages.selectors.quickEditOverlay).removeClass('active').remove();
	...
	{
		$(nestpages.selectors.ajaxError).find('p').text(message);
		$(nestpages.selectors.ajaxError).show();
	}
	...
	{
		$(nestpages.selectors.ajaxError).hide();
	}
	...
	{
		var thumbnail = $(nestpages.selectors.thumbnailContainer).not(nestpages.selectors.thumbnailContainerLink).first().find('img');
		var width = $(thumbnail).width();
		var height = $(thumbnail).height();
		$.each($(nestpages.selectors.thumbnailContainerLink), function(){
			$(this).width(width);
			$(this).height(height);
		});
	}
...
nestpages.Dropdowns = function()
{
	...
	self.bindEvents = function()
	{
		...
		$(document).on('dropdown-opened', function(e, content){
			if ( $(content).parents(nestpages.selectors.row).length > 0 ){
				$(content).parents(nestpages.selectors.row).addClass('active');
			}
		});
		$(document).on('dropdown-closed', function(){
			$(nestpages.selectors.row).removeClass('active');
		});
	}
...
/**
* Check All functionality for Nest Pages
* @package Nest Pages
* @author Kyle Phillips - https://github.com/kylephillips/wp-nested-pages
*/
nestpages.CheckAll = function()
{
	var plugin = this;
	var $ = jQuery;
	...
	plugin.toggleCheckboxes = function()
	{
		...
		$.each(checkboxes, function(){
			...
			if ( $(row).hasClass('np-hide') && !$(row).is(':visible') ) {
				$(row).find(nestpages.selectors.bulkActionsCheckbox).prop('checked', false)
			};
		});
	...
	*/
	plugin.toggleCheckAll = function()
	{
		...
		var hidden_checkboxes = $('.np-hide').find(nestpages.selectors.bulkActionsCheckbox).length;
		var hidden_checkboxes_visible = $('.np-hide:visible').find(nestpages.selectors.bulkActionsCheckbox).length;
...
nestpages.NewPost = function()
{
	...
	plugin.bindEvents = function()
	{
		$(document).on('open-modal', function(e, button, modal){
			var target = $(button).attr('data-nestpages-modal-toggle');
			if ( typeof target !== 'undefined' && target == 'np-bulk-modal' ){
				plugin.openModal();
			}
		});
		$(document).on('submit', nestpages.selectors.newPageForm, function(e){
			e.preventDefault();
		});
		$(document).on('click', nestpages.selectors.newPageSubmitButton, function(e){
			e.preventDefault();
			plugin.submitForm($(this));
		});
		$(document).on('click', nestpages.selectors.newPageTitle, function(e){
			e.preventDefault();
			plugin.addTitleField($(this));
		});
		$(document).on('click', nestpages.selectors.newPageRemoveTitle, function(e){
			e.preventDefault();
			plugin.removeTitleField($(this));
		});
		$(document).on('click', nestpages.selectors.addChildButton, function(e){
			e.preventDefault();
			plugin.openQuickEdit($(this));
		});
		$(document).on('click', nestpages.selectors.cancelNewChildButton, function(e){
			e.preventDefault();
			plugin.cancelNewPage();
		});
		$(document).on('click', '[' + nestpages.selectors.newBeforeButton + ']', function(e){
			e.preventDefault();
			plugin.openQuickEdit($(this));
		});
		$(document).on('click', '[' + nestpages.selectors.newAfterButton + ']', function(e){
			e.preventDefault();
			plugin.openQuickEdit($(this));
		});
	...
	{
		var newform = $(nestpages.selectors.newPageFormContainer).clone().find(nestpages.selectors.newPageForm);
		$(newform).addClass('in-modal');
		$(nestpages.selectors.newPageModal).find('.modal-body').html(newform);
		$(nestpages.selectors.newPageModal).find('h3').text(nestpages.add_multiple);
		$(nestpages.selectors.newPageModal).find('.page_parent_id').val(plugin.parent_id);
		$(newform).find('.np_title').first().focus();
		$(newform).find(nestpages.selectors.newPageTitle).first().prop('tabindex', '2');
	}

	// Open the new child quick edit
	plugin.openQuickEdit = function(button)
	{
		var before = $(button).attr(nestpages.selectors.newBeforeButton);
		before = ( typeof before === 'undefined' || before === '' ) ? false : before;

		var after = $(button).attr(nestpages.selectors.newAfterButton);
		after = ( typeof after === 'undefined' || after === '' ) ? false : after;

		var parent_li = $(button).closest(nestpages.selectors.row).parent('li');
		var newform = $(nestpages.selectors.newPageFormContainer).clone();
		...
		$(newform).siblings(nestpages.selectors.row).hide();
		...
		if ( before ) {
			...
			$(newform).find('[data-new-post-relation-title]').text(nestpages.insert_before + ': ' + $(button).attr('data-parentname'));
		}
		if ( after ) {
			$(newform).find('.page_after_id').val(after);
			$(newform).find('[data-new-post-relation-title]').text(nestpages.insert_after + ': ' + $(button).attr('data-parentname'));
		}

		$(newform).show();
		$(newform).find('.np_title').focus();
		$(newform).find(nestpages.selectors.newPageTitle).prop('tabindex', '2');
	}
	...
	plugin.cancelNewPage = function()
	{
		...
		$(nestpages.selectors.newChildError).hide();
		$(nestpages.selectors.newPageModal).find('.modal-body').empty();
		$(nestpages.selectors.sortable).find('.new-child').remove();
		$(nestpages.selectors.row).show();
	}
	...
	plugin.submitForm = function(button)
	{
		...
		var action = nestpages.formActions.newPage;
		if ( $(plugin.form).find('.page_before_id').val() !== '' ) action = nestpages.formActions.newBeforeAfter;
		if ( $(plugin.form).find('.page_after_id').val() !== '' ) action = nestpages.formActions.newBeforeAfter;
		
		$.ajax({
			url: nestpages.jsData.ajaxurl,
			type: 'post',
			datatype: 'json',
			data: $(plugin.form).serialize() + '&action=' + action + '&nonce=' + nestpages.jsData.nonce + '&syncmenu=' + nestpages.jsData.syncmenu + '&post_type=' + nestpages.jsData.posttype,
			success: function(data){
				if (data.status === 'error'){
					plugin.toggleLoading(false);
					$(plugin.form).find(nestpages.selectors.quickEditErrorDiv).text(data.message).show();
					return;
				}
			...
			error: function(data){
				console.log(data);
				plugin.toggleLoading(false);
				$(plugin.form).find(nestpages.selectors.quickEditErrorDiv).text('The form could not be saved at this time.').show();
			}
		});
	}
	...
	plugin.appendRows = function(appendto, post, before, after)
	{
		...
		if ( nestpages.jsData.hierarchical ){
			html += '<div class="row">';
		...
		html += '<div class="nestpages-dropdown" data-dropdown><a href="#" class="np-btn has-icon toggle" data-dropdown-toggle><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M6 10c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm12 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm-6 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg></a><ul class="nestpages-dropdown-content" data-dropdown-content>';
		// Add Link
		html += '<li><a href="#" class="open-redirect-modal" data-parentid="' + post.id + '"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/></svg>' + nestpages.add_link + '</a></li>';
		// Add Child
		html += '<li><a href="#" class="add-new-child" data-id="' + post.id + '" data-parentname="' + post.title + '"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M3 21h18v-2H3v2zM3 8v8l4-4-4-4zm8 9h10v-2H11v2zM3 3v2h18V3H3zm8 6h10V7H11v2zm0 4h10v-2H11v2z"/><path d="M0 0h24v24H0z" fill="none"/></svg>' + nestpages.add_child_short + '</a></li>';
		html += '</ul></div>';
		
		// Quick Edit (data attrs)
		html += '<a href="#" class="np-btn np-quick-edit" data-id="' + post.id + '" data-template="' + post.page_template + '" data-title="' + post.title + '" data-slug="' + post.slug + '" data-commentstatus="closed" data-status="' + post.status.toLowerCase() + '" data-np-status="show"	data-navstatus="show" data-author="' + post.author + '" data-template="' + post.template + '" data-month="' + post.month + '" data-day="' + post.day + '" data-year="' + post.year + '" data-hour="' + post.hour + '" data-minute="' + post.minute + '" data-datepicker="' + post.datepicker + '" data-time="' + post.time + '" data-formattedtime="' + post.formattedtime + '" data-ampm="' + post.ampm + '">' + nestpages.quick_edit + '</a>';

		html += '<a href="' + post.view_link + '" class="np-btn" target="_blank">' + nestpages.view + '</a>';
	...
	{
		var row = $(nestpages.selectors.rows + '#menuItem_' + id);
		return row;
	}
	...
	plugin.toggleAddEditButton = function(form)
	{
		...
		if ( titleCount < 1 ){
			$(nestpages.selectors.newPageSubmitButton).hide();
			return;
		}
		$(nestpages.selectors.newPageSubmitButton).show();
		if ( titleCount > 1 ){
			$(nestpages.selectors.newPageSubmitButton + '.add-edit').hide()
			return;
		}
		$(nestpages.selectors.newPageSubmitButton + '.add-edit').show()
	}
	...
	{
		if ( loading ){
			$(nestpages.selectors.quickEditErrorDiv).hide();
			$(nestpages.selectors.newPageSubmitButton).attr('disabled', 'disabled');
			$(nestpages.selectors.quickEditLoadingIndicator).show();
			return;
		}
		$(nestpages.selectors.newPageSubmitButton).attr('disabled', false);
		$(nestpages.selectors.quickEditLoadingIndicator).hide();
	}
...
* @author Kyle Phillips - https://github.com/kylephillips/wp-nested-pages
*/
nestpages.QuickEditPost = function()
{
	var plugin = this;
	var $ = jQuery;
	...
	plugin.parent_li = ''; // The post's Nest Pages list element
	...
	plugin.bindEvents = function()
	{
		$(document).on('click', nestpages.selectors.quickEditOpen, function(e){
			e.preventDefault();
			plugin.button = $(this);
			plugin.openForm();
		});
		$(document).on('click', nestpages.selectors.quickEditCancel, function(e){
			e.preventDefault();
			plugin.formatter.removeQuickEdit();
		});
		...
		$(document).on('click', nestpages.selectors.quickEditSaveButton, function(e){
			e.preventDefault();
	...
	plugin.setInitialData = function()
	{
		...
		plugin.parent_li = $(plugin.button).closest(nestpages.selectors.row).parent('li');
	...
	{
		plugin.form = $(nestpages.selectors.quickEditPostForm).clone();
		...
		$(plugin.form).siblings(nestpages.selectors.row).hide();
		$(plugin.form).show();
	}
		...
		$.each(datepickers, function(){
			...
			$this.datepicker({
				...
				beforeShow: function(input, inst) {
					$('#ui-datepicker-div').addClass('nestpages-datepicker');
				}
			});
		});
	...
	plugin.getTermNames = function()
	{
		$.ajax({
			...
			datatype: 'json',
			data : {
				action : nestpages.formActions.getTaxonomies,
				nonce : nestpages.jsData.nonce,
	...
	plugin.save = function()
	{
		...
		$.ajax({
			...
			type: 'post',
			datatype: 'json',
			data: $(plugin.form).find('form').serialize() + '&action=' + nestpages.formActions.quickEditPost + '&nonce=' + nestpages.jsData.nonce + '&syncmenu=' + nestpages.jsData.syncmenu + '&post_type=' + nestpages.jsData.posttype,
			success: function(data){
				if (data.status === 'error'){
					plugin.toggleLoading(false);
					$(plugin.form).find(nestpages.selectors.quickEditErrorDiv).text(data.message).show();
	...
	plugin.updatePostRow = function()
	{
		...
		if ( (plugin.newData._status !== 'publish') && (plugin.newData._status !== 'future') ){
			var newStatus = nestpages.post_statuses[plugin.newData._status].label;
			$(status).text('(' + newStatus + ')');
		} else {
			$(status).text('');
		}
		if ( plugin.newData.keep_private === 'private' ){
			$(status).text(nestpages.private);
		}
		...
		var button = $(plugin.row).find(nestpages.selectors.quickEditOpen);
...
* @author Kyle Phillips - https://github.com/kylephillips/wp-nested-pages
*/
nestpages.QuickEditLink = function()
{
	var plugin = this;
	var $ = jQuery;
	...
	plugin.bindEvents = function()
	{
		$(document).on('click', nestpages.selectors.quickEditButtonLink, function(e){
			e.preventDefault();
			plugin.formatter.removeQuickEdit();
			plugin.button = $(this);
			plugin.openQuickEdit();
		});
		$(document).on('click', nestpages.selectors.quickEditLinkSaveButton, function(e){
			e.preventDefault();
			plugin.submitForm();
		});
		$(document).on('keydown', function(e){
	...
	plugin.createForm = function()
	{
		var parent_li = $(plugin.button).closest(nestpages.selectors.row).parent('li');
		plugin.form = $(nestpages.selectors.quickEditLinkForm).clone();
		...
		var row = $(plugin.form).siblings(nestpages.selectors.row).hide();
	}
	...
	plugin.submitForm = function()
	{
		...
		$.ajax({
			...
			type: 'post',
			datatype: 'json',
			data: $(plugin.form).find('form').serialize() + '&action=' + nestpages.formActions.quickEditLink + '&nonce=' + nestpages.jsData.nonce + '&syncmenu=' + nestpages.jsData.syncmenu + '&post_type=' + nestpages.jsData.posttype,
			success: function(data){
				if (data.status === 'error'){
					plugin.toggleLoading(false);
					$(plugin.form).find(nestpages.selectors.quickEditErrorDiv).text(data.message).show();
	...
	plugin.updateRow = function()
	{
		...
		if ( (plugin.newPostData.nav_status == 'hide') ){
			$(nav_status).text('(' + nestpages.jsData.hiddenText + ')');
		} else {
			$(nav_status).text('');
		}
		...
		var button = $(row).find(nestpages.selectors.quickEditButtonLink);
	...
	plugin.toggleLoading = function(loading)
	{
		...
		if ( loading ){
			$(nestpages.selectors.quickEditErrorDiv).hide();
			$(nestpages.selectors.quickEditLinkSaveButton).attr('disabled', 'disabled');
			$(nestpages.selectors.quickEditLoadingIndicator).show();
			return;
		}
		$(nestpages.selectors.quickEditLinkSaveButton).attr('disabled', false);
		$(nestpages.selectors.quickEditLoadingIndicator).hide();
	}
...
nestpages.MenuLinks = function()
{
	...
	plugin.search = new nestpages.MenuSearch;

	plugin.bindEvents = function()
	{
		$(document).on('click', nestpages.selectors.openLinkModal, function(e){
			e.preventDefault();
	...
	plugin.openModal = function()
	{
		$(nestpages.selectors.linkErrorDiv).hide();
		$(nestpages.selectors.linkModal).find('input').val('');
		$(nestpages.selectors.linkModal).find('.parent_id').val(plugin.postParent);
		plugin.clearForm();
		$(plugin.selectors.accordion).find('ul').hide();
		$(plugin.selectors.typeSelect).removeClass('active');
		$(document).trigger('open-modal-manual', nestpages.selectors.linkModal);
	}
	...
	plugin.submitForm = function()
	{
		...
		$.ajax({
			url : nestpages.jsData.ajaxurl,
			type : 'post',
			data: $(plugin.selectors.form).serialize() + '&action=' + nestpages.formActions.newMenuItem + '&nonce=' + nestpages.jsData.nonce + '&post_type=' + nestpages.jsData.posttype + '&syncmenu=' + nestpages.jsData.syncmenu,
			success : function(data){
				plugin.toggleLoading(false);
				if ( data.status === 'error' ){
	...
	// Create the Nest Pages row for the new link
	plugin.createRow = function()
	{
		...
		html += nestpages.jsData.quickEditText;
		html += '</a>';
	...
	plugin.toggleLoading = function(loading)
	{
		if ( loading ){
			$(plugin.selectors.form).find(plugin.selectors.errorDiv).hide();
			$(plugin.selectors.form).find(nestpages.selectors.quickEditLoadingIndicator).show();
			$(plugin.selectors.saveButton).attr('disabled', 'disabled');
			return;
		}
		$(plugin.selectors.form).find(nestpages.selectors.quickEditLoadingIndicator).hide();
...
nestpages.MenuSearch = function()
{
	...
	plugin.query = function()
	{
		...
		$.ajax({
			...
			datatype: 'json',
			data: {
				action : nestpages.formActions.search,
				nonce : nestpages.jsData.nonce,
...
var nestpagesTrash = function()
{
	...
	// JS Data
	plugin.nonce = nestpages.np_nonce;
...
* @author Kyle Phillips - https://github.com/kylephillips/wp-nested-pages
*/
nestpages.PostSearch = function()
{
	var plugin = this;
	var $ = jQuery;

	plugin.selectors = {
		input : 'data-nestpages-post-search',
		form : 'data-nestpages-post-search-form',
		loadingIndicator : 'data-nestpages-loading',
		noResults : 'data-nestpages-no-results',
		results: 'data-nestpages-search-results'
	}
	...
	*/
	...
	plugin.query = function()
	{
		...
		$.ajax({
			...
			datatype: 'json',
			data: {
				action : nestpages.formActions.postSearch,
				nonce : nestpages.jsData.nonce,
