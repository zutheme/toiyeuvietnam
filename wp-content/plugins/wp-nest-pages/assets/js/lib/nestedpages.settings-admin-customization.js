...
*/
nestpages.AdminCustomizationSettings = function()
{
	var plugin = this;
	var $ = jQuery;

	plugin.selectors = {
		navItemCheckbox : 'data-nestpages-admin-nav-item-checkbox',
	...
	*/
	plugin.addSeparator = function()
	{
		...
		html += '</div><!-- .menu-item -->'
		html += '<input type="hidden" name="nestpages_admin[nav_menu_options][' + currentRole + '][custom_sep' + separatorCount + ']" value="true">';
		html += '<input type="hidden" name="nestpages_admin[nav_menu_options][' + currentRole + '][custom_sep' + separatorCount + '][order]" value="" data-np-menu-order>';
		html += '</li>';
...
new nestpages.AdminCustomizationSettings;