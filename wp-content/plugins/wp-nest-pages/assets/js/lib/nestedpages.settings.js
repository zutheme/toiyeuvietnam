...
*/
nestpages.Settings = function()
{
	var plugin = this;
	var $ = jQuery;

	plugin.selectors = {
		postTypeToggle : '[data-toggle-nestpages-pt-settings]', // Toggle Button for Post Type Settings
		postTypeCheckbox : '[data-nestpages-settings-row-checkbox]', // Checkbox for enabling post type
		customFieldsCheckbox : '[data-toggle-nestpages-cf-settings]', // Checkbox for toggling custom fields settings
		standardFieldsCheckbox : '[data-toggle-nestpages-sf-settings]', // Checkbox for toggling standard field settings
		...
		assignPostTypeCheckbox : '[data-nestpages-assign-post-type]', // Checkbox for assigning a post type to a page
		...
		assignPostTypeRemove : '[data-nestpages-page-pt-assignment-remove]', // Link to remove the assigned page for the post type,
		assignPostTypeSelection : '[data-nestpages-page-pt-assignment-selection]', // The div displaying the selection

		// Sort Options for Post Types
		sortOptionCheckbox : '[data-nestpages-sort-option-checkbox]', // Checkbox for enabling a sort option
		defaultSortOptions : '[data-nestpages-sort-option-default]', // Default sort options (containing div)
	}
	...
	*/
	plugin.toggleAssignPostType = function()
	{
		...
		$.each(checkboxes, function(){
			var checkbox = $(this);
			var listing = $(this).parents('.field').find('.nestpages-assignment-display');
			if ( $(checkbox).is(':checked') ){
				$(listing).show();
			} else {
				$(listing).hide();
	...
	*/
	plugin.chooseAssignPostType = function(element)
	{
		...
		var html = nestpages.currently_assigned_to + ' ' + pageTitle + ' <a href="#" data-nestpages-page-pt-assignment-remove>(' + nestpages.remove + ')</a>';
		...
		$(container).find('[data-nestpages-post-search-form]').hide();
	}
	...
	*/
	plugin.removeAssignPostType = function(element)
	{
		...
		$(container).find('[data-nestpages-post-search-form]').show();
