...
Nest Pages provides a drag and drop interface for managing pages & posts in the WordPress admin, while maintaining quick edit functionality.
...
For more information visit [nestpages.com](http://nestpages.com).

**Important: Nest Pages requires WordPress version 3.8 or higher, and PHP version 5.4 or higher.**
...
3. Click on the Pages Menu item to begin ordering pages. Nest Pages replaces the default Page management screen.
4. To access the default the pages screen, select Default Pages located in the Pages submenu, or on the Nest Pages screen
5. Additional options are available through the plugin settings at Settings > Nest Pages. Nest Pages may be enabled for any post type, and is configurable by post type. Options include quick edit field settings, thumbnail display and more.

== Frequently Asked Questions ==

= Can I use Nest Pages with other post types? =
Yes! Visit Settings > Nest Pages > Post Types to enable the interface for any public post type. There are settings available for each post type, allowing customization of the admin to fit your needs.

= Is this plugin compatible with the WPML plugin? =
Please see the section titled "WMPL Compatibility" for details on using Nest Pages with WPML. In short, ordering and nesting functionality is available, but automatic menu generation across languages is limited. It is advised to use WPML's menu synchronization for this purpose.

= How do I access the WordPress "Pages" screen? =
Click the “Default link in the page subnav, or on the Nest Pages screen. This item may be optionally hidden under the Nest Pages "Post Type" settings.
...
Custom columns are not currently supported by Nest Pages. To view custom columns, click on the "Default" link to view the native interface. If you are using WordPress SEO by Yoast, a page analysis indicator is shown.
...
Yes! Visit Settings > Nest Pages > Post Types to configure thumbnail settings for each post type. Filters are also available for customizing the images displayed, as well as specifying a fallback image in the case that the post does not have a thumbnail assigned.

= Nest Pages changes my permalink structure. How do I stop that? =
Nest Pages uses the same ordering methodology as WordPress core. The plugin offers an enhanced interface to achieve the same results. Parent/Child relationships are used in conjunction with the post menu_order field to nest and order posts/pages. There is currently no option to disable this core feature.
...
9. The Nest Pages interface can be enabled on a per-post-type basis, with customizable options for each type.
...
= 3.1.9 =
* Adds a custom sorting user capability for each public post type: nestpages_sort_$type, enabling control of capabilities through 3rd party plugins. 
* Adds filter for sorting capability: nestpages_sort_capability. Filter passes 3 parameters: $grant_capability (boolean), $type (post type name), and $role (user role object)
* Adds a plugin option for disabling the sort view based on user role.
* Adds filters for each post type to allow/disable the sort view based on user role and post type: nestpages_sort_view_$type. Filter returns 2 parameters: $grant_access (boolean), and $roles (array of roles assigned to the current user).
...
= 3.1.4 =
* Adds support for custom statuses, configurable by post type. To enable statuses for a specific post type, visit the plugin settings: Settings > Nest Pages > Post Types. Toggle the post type. If there are custom statuses available, a field titled "Enable Custom Statuses" will be available to select statuses.
...
= 3.1.3 =
* Updates nesting interface to use an indented style. To revert to the previous format, visit Settings > Nest Pages and select the checkbox titled "Use the classic (non-indented) hierarchy display."
...
= 3.1.1 =
* Removes ability to hide Settings and Nest Pages menus from administrators within the admin customization interface (removing items hides links to necessary admin sections).
...
= 3.1.0 =
* Adds new feature for customizing the order, visibility, and labels of the admin menu. To view the feature, visit Settings > Nest Pages > Admin Customization > Admin Menu. Important: once custom menus have been configured, new menu items added by other plugins may not appear upon activation. To add these items, revisit the Nest Pages settings and reconfigure the menus with the new items (they will appear at the bottom of the customized menu in the drag and drop interface). While menus may be configured for each user group, the plugin does not set any permissions. While items may be hidden from the menu, they will still be accessible with a direct link if the user has the appropriate permissions.
...
= 3.0.5 = 
* Adds actions for sorting update. Single post: nestpages_post_order_updated($post_id, $parent, $order). All posts: nestpages_posts_order_updated($posts, $parent).
* Adds filter to disable sorting per post: nestpages_post_sortable($sortable, $post, $post_type).
...
= 3.0.1 =
* Breaking Change: PHP Version 5.4.0+ is required to run Nest Pages V3+.
...
* Adds error handling for existing nav menu with the name “Nest Pages”, preventing WP_Error object from saving in the database which causes a fatal error in some instances.
...
* Important upgrade note: Custom links within the Nest Pages interface are disabled if WPML is installed and enabled. This is a potential breaking change, so please remove all custom links before updating. Links may be added back to menus by using the default WordPress Appearance > Menus editor.
* Sort/Order options on non-page post type listings may now be added/removed. All options are hidden by default. To enable these, visit Settings > Nest Pages > Post Types. Under the desired post type, options may be enabled under the "Sort Options" row.
* Bug fix where pages hidden from the Nest Pages view where displaying.
...
* Bug fix where selecting "Hide in Nest Pages" was not saving in the quick edit interface.
...
* Custom columns are now supported. Visit Settings > Nest Pages > Post Types to enable and order custom columns for each post type. Columns made available through the appropriate WordPress filters are available for selection, along with any taxonomies enabled for the post type.
...
* Thumbnail support added to nested/sort view. Visit Settings > Nest Pages > Post Types to enable post thumbnails and set options.
* Ability to customize quick edit fields for each post type added. Visit Settings > Nest Pages > Post Types to hide specific fields from the quick edit interface for each post type. Please note: specific fields are still hidden depending. on the current user's roles/capabilities.
...
= 1.5.0 =
* Links now include all taxonomies/post types, enabling full control over the primary site menu from the Nest Pages interface. Start adding menu items by selecting "Add Link" from the top, or the link button on a specific row to add a child item.
...
* Option added to hide the "Sync Menu" checkbox (visit Settings > Nest Pages > General to hide)
...
= 1.3.10 =
* Bug fix - resolves deprecated function issue with SEO by Yoast update v1.7.3. Critical for sites running both Nest Pages and WordPress SEO by Yoast

= 1.3.9 =
* Bug fix - error when deleting a page from the nested view with menu sync disabled

= 1.3.8 =
* Bug fix – Critical error that was overriding existing menu items outside of the Nest Pages generated menu. Other menus are now unaltered on save.
...
* Changes to page and link menu items under appearance > menu now sync the Nest Pages listing when menu sync is enabled (other custom menu item types not yet supported).
...
= 1.3.0 =
* All public post types are now supported, both hierarchical and non-hierarchical. To enabled the Nest Pages interface for additional post types, visit Settings > Nest Pages and select the "Post Types" tab. The generated nav menu is tied to the pages type, which is enabled by default.
...
= 1.2.0 =
* PHP 5.3.2+ is now required to run Nest Pages. Will not run or install on older versions of PHP.
...
= 1.1.8 =
* New Child Pages Interface - Add child pages more efficiently with the new add child pages dialog. Add a single child page without leaving the Nest Pages view, or add multiple pages with one click. Reorder multiple child pages before saving with the drag and drop interface you're accustomed to.
...
* Trashing pages now redirects to Nest Pages view
...
= 1.3.10 =
Resolves deprecated function issue with SEO by Yoast update v1.7.3. Critical for sites running both Nest Pages and WordPress SEO by Yoast
...
= 1.3.1 =
Date fields in Quick Edit windows are now replaced with a date picker and formatted time. If the formatting conflicts with your specific locale, disable the datepicker under Settings > Nest Pages > General.
...
= 1.2 =
PHP 5.3.2 now required – Nest Pages will not install on older versions of PHP. If you are running less than 5.3.2, continue to use Nest Pages version 1.1.9.
...
= Generated Menu =
The default menu generated automatically is named "Nest Pages". You may rename the menu under Appearance > Menus, or under the Nest Pages settings.


= Toggling the Page Tree =

To toggle the child pages in and out of view, click the arrow to the left of a parent page. To quickly expand and collapse all pages, click the button in the upper right corner of the Nest Pages Screen. 


= Theme Use =

To order by Nest Pages ordering in your theme, use the `menu_order` order option in your queries. 


= Hiding Pages from the Tree View =

To hide a page from the tree view, open the quick edit form, select the option to “Hide in Nest Pages” and click Update to save the change.
...
After installing Nest Pages, a new menu will be available with the name `Nest Pages`. By default, menu syncing is enabled. To disable the sync, uncheck “Sync Menu” at the top of the Nest Pages screen. Recheck the box to enable it again and to run the sync. 

**Saving Performance:** If your site has a very large number of pages, disabling page sync may help speed up the save time when using Nest Pages.

**Editing the generated menu:** Any manual changes made to the menu outside of the Nest Pages interface will be overwritten after the synchronization runs.

**Hiding Pages in the Menu:** To hide a page from the Nest Pages menu, click “Quick Edit” on it’s row, select “Hide in Nav Menu”, and click “update”. If menu sync is disabled, enable it now to sync the setting. Hidden pages are marked “(Hidden)”. If a page with child pages is hidden from the menu, all of it’s child pages will also be hidden. 


= WPML Compatibility =

As of version 2.0.1, some features of Nest Pages are compatible with WPML. There are some important exceptions to take note of before upgrading to version 2 if WPML is installed, or if installing WPML on an existing Nest Pages enabled site.

**WPML and menu support:** Certain features within Nest Pages are disabled if WPML is installed and enabled. This is due to the complexity of menu synchronization across languages. While automatic menu sync remains available for the site's primary language, additional languages must be synchronized using WPML's menu synchronization methods. If menu sync is enabled within Nest Pages, a "Sync WPML Menus" link is available on non-primary language screens for convenience.

**Custom Links:** Custom links within Nest Pages are disabled on installs with WPML. Custom links may be added through the traditional Appearance > Menus interface. This ensures that WPML menus synchronize successfully across languages.
...
* `nestpages_thumbnail($image, $post)` – Customize the thumbnail for each page/post. Note: Thumbnails must be enabled for the post type.
* `nestpages_thumbnail_fallback($image, $post)` - Customize the thumbnail fallback for each page/post (if the post does not have a featured image). Note: Thumbnails must be enabled for the post type.
* `nestpages_edit_link($link, $post)` - Customize the "edit" link for each page/post in the nested interface.
* `nestpages_edit_link_text($text, $post)` - Customize the "edit" link text for each page/post in the nested interface.
* `nestpages_view_link($link, $post)` - Customize the "view" button link for each page/post in the nested interface.
* `nestpages_view_link_text($text, $post)` - Customize the "view" button text for each page/post in the nested interface.